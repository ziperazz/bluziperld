...
import {
  ...
  ShoppingCart, Search, AlertTriangle
} from "lucide-react";
...
export default function ModalProductForm({ isOpen, onClose, editData, onSave }: ModalProductFormProps) {
    ...
    if (editData) {
      setFormData({
        ...
        images: (editData.images || []).filter(
          (img) => typeof img === "string" && img.trim() !== "" && img.trim() !== "/placeholder.png" && img.trim() !== "placeholder.png"
        ),
        imageFiles: [],
      });
  ...
  function validate() {
    ...
    if (formData.discount < 0 || formData.discount > 100) errs.discount = "تخفیف باید بین 0 تا 100 باشد";
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => {
    ...
    const numberFields = ["price", "instock", "purchaseCount", "ratingAverage", "rating", "discount", "visits"];
          ...
          {activeTab === "basic" && (
            <div className="p-6 space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <Input label="عنوان محصول" name="title" value={formData.title} onChange={handleChange} error={errors.title} icon={Tag} />
          ...
          {activeTab === "media" && (
            <div className="p-6 space-y-6">
              ...
              <SpecsManager
                ...
                onRemove={(idx: number) => updateField("specs", formData.specs.filter((_, i) => i !== idx))}
              />
            </div>
          )}
...
function ExistingImagesManager({ images, onRemove }: { images: string[]; onRemove: (idx: number) => void }) {
  const apiBase = process.env.NEXT_PUBLIC_API_URL?.replace(/\/+$/, "") || "http://localhost:5000";

  const buildImageUrl = (img: string) => {
    if (!img) return "/placeholder.png";
    if (/^https?:\/\//i.test(img)) return img;
    if (!img.startsWith("/uploads")) return `${apiBase}/uploads/${img.replace(/^\/+/, "")}`;
    return `${apiBase}${img}`;
  };
