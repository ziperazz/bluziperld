...
export default function SupportTicketPage(props: { params: { id: string } }) {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center px-4">
        <div className="text-center">
          ...
          <Link href="/support" className="inline-block mt-4 text-blue-400 text-sm hover:text-blue-300">
            بازگشت به پشتیبانی
          </Link>
        </div>
      </div>
  ...
  return (
    <div className="min-h-screen px-4 py-6 md:py-8">
      <div className="max-w-4xl mx-auto h-[85vh] md:h-[80vh] flex flex-col">
        ...
        <nav className="flex items-center gap-2 text-[10px] md:text-xs text-gray-500 mb-4 shrink-0">
          ...
          <Link href="/support" className="hover:text-blue-400 transition-colors flex items-center gap-1">
            <HeadphonesIcon size={11} /> پشتیبانی
          </Link>
          <ChevronRight size={10} />
        ...
        >
          {/* ============ HEADER ============ */}
          <div className="shrink-0 px-4 md:px-5 py-3 border-b border-white/[0.04] flex items-center justify-between bg-[#050912]/80 backdrop-blur-xl">
            <div className="flex items-center gap-3 min-w-0">
              <div className="relative shrink-0">
                <div className="w-9 h-9 md:w-10 md:h-10 rounded-2xl bg-gradient-to-br from-blue-500/20 to-cyan-500/20 flex items-center justify-center border border-blue-500/10">
                  <Shield size={16} className="text-blue-400" />
                </div>
                <div className={`absolute -bottom-0.5 -right-0.5 w-3 h-3 rounded-full border-2 border-[#050912] ${adminOnline ? "bg-green-400" : "bg-gray-500"}`} />
              </div>
                ...
                <div className="flex items-center gap-1.5 mt-0.5">
                  {adminOnline ? (
                    <>
                      ...
                      <span className="text-[9px] md:text-[10px] text-green-400 font-medium">پشتیبان آنلاین</span>
                    </>
                  ) : (
                    <>
                      <Clock size={9} className="text-gray-500" />
                      <span className="text-[9px] md:text-[10px] text-gray-500">پاسخگویی ۹ صبح تا ۹ شب</span>
                    </>
                  )}
                </div>
              </div>
            ...
            <div className={`shrink-0 px-2.5 py-1 md:px-3 md:py-1.5 rounded-xl text-[9px] md:text-[10px] font-bold ${
              isClosed
          ...
          <div ref={chatRef} className="flex-1 overflow-y-auto px-3 md:px-5 py-4 md:py-5 space-y-3 md:space-y-4">
            ...
            {ticket.messages.length === 0 && (
              <div className="flex flex-col items-center justify-center py-16 text-center">
                ...
                <p className="text-[10px] md:text-[11px] text-gray-600 mt-1">پیام خود را بنویسید و ارسال کنید</p>
              </div>
            )}

            {ticket.messages.map((msg, i) => {
              const mine = msg.senderRole === "USER"
                  ...
                  <div className={`flex items-end gap-2 max-w-[88%] md:max-w-[80%] ${mine ? "flex-row-reverse" : "flex-row"}`}>
                    ...
                    <div className={`px-3 md:px-4 py-2 md:py-2.5 rounded-2xl text-xs md:text-sm leading-6 md:leading-7 ${
                      ...
                      <div className={`flex items-center gap-1.5 mt-1.5 ${mine ? "justify-end" : "justify-start"}`}>
                        ...
                        {mine && (msg.seen ? <CheckCheck size={10} className="text-blue-200 opacity-70" /> : <Check size={10} className="text-white opacity-50" />)}
                      </div>
                    </div>
                  </div>
                </div>
              ...
              <div className="flex items-end gap-2 md:gap-3">
                <div className="flex-1 relative">
                  <textarea
                    ...
                    rows={1}
                    className="w-full bg-white/[0.03] border border-white/[0.06] rounded-2xl px-3 md:px-4 py-2.5 md:py-3 pr-10 text-xs md:text-sm text-white placeholder:text-gray-600 focus:border-blue-500/30 focus:bg-white/[0.05] outline-none transition-all resize-none max-h-[120px] leading-6"
                  />
                </div>
                <button
                  onClick={sendMessage}
                  ...
                  className={`shrink-0 w-10 h-10 md:w-11 md:h-11 rounded-2xl flex items-center justify-center transition-all duration-300 ${
                    message.trim() && !sending
                      ? "bg-gradient-to-br from-blue-600 to-cyan-600 text-white shadow-lg shadow-blue-500/30 hover:scale-105 active:scale-95"
